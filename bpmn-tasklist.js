...
const nonEmptyString = Match.Where(s => typeof s === 'string' && s.length > 0)
...
tasklistHooks.onExecuteBefore = function (engineFct, options) {
  ...
  tasklistListener.on('wait', Meteor.bindEnvironment((userTask) => {
    const innserEngine = engineFct()
    const {instanceId} = innserEngine
    tasklist.add(instanceId, clean(userTask.activity))
  }))

  tasklistListener.on('end-userTask', Meteor.bindEnvironment((userTask) => {
